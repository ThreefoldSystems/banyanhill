<?php
	...
	//[bh_signup_form buttontext="Button Text" emailtext="Email Text" xcode="XCODE123" position="CSS_CLASS" multivariate="BOOLEAN"]
	function bh_signup_form_func($atts) {
		...
		$markup .= 'buttontext='.$a['buttontext'].'&';
		$markup .= 'emailtext='.$a['emailtext'].'&';
		$markup .= 'xcode='.$a['xcode'].'&';
		$markup .= $a['position'] ? 'position='.$a['position'].'&' : '';
		$markup .= $a['multivariate'] === 'true' ? 'multivariate='.$a['multivariate'].'&' : '';		
	...
	//Add a button to the tinyMCE visual editor
	...
	function register_button($buttons) {
	   array_push($buttons, "bh_quote", "bh_accordion");
	...
	function be_load_more_js() {
		...
		$query = array( 
			...
			'category_name' => get_post_primary_category($post->ID)['primary_category']->slug, 
			'posts_per_page' => 1 
		);
...
// 		add_image_size( 'mam-email-image', 250 , 167, true );
// 	}
...
//	}
	...
	function tfs_bh_experts( $atts = null ) {
		...
		$arguments = array(
			...
			'posts_per_page' => -1,
			'orderby'        => 'rand',
			'post__not_in' => array( $post->ID, 102710 )       
		);
	...
	function acf_load_experts_subscription_connection( $field ) {
		...
		if ( $get_experts ) {
			foreach ( $get_experts as $expert ) {
				$field['choices'][$expert->ID] = $expert->post_name;
			}
		}
	...
	<?php
		wp_dropdown_users( array(
			'role__in' => [ 'author', 'contributor', 'editor' ], // Add desired roles here.
	...
	function get_reg_date() {
		return date( "F j, Y", strtotime( get_userdata( wp_get_current_user()->ID )->user_registered ) );
	}
