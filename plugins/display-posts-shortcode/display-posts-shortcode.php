 ...
 * @version 3.0.2
 * @author Bill Erickson <bill@billenrickson.net>
...
function be_display_posts_shortcode( $atts ) {
	...
	$bh_infinite_scroll    = (boolean) $atts['infinite_scroll'];
	...
	if ( ! empty( $bh_infinite_scroll ) ) {
		$args['infinite_scroll'] = $bh_infinite_scroll;
	}
	...
	while ( $dps_listing->have_posts() ) :
		...
		if ( $include_title && $include_link ) {
			...
			$title = '<a class="title" href="' . apply_filters( 'the_permalink', get_permalink() ) . '">' . wp_kses_post( get_the_title() ) . '</a>';

		} elseif ( $include_title ) {
			$title = '<span class="title">' . wp_kses_post( get_the_title() ) . '</span>';
		...
		if ( $image_size && has_post_thumbnail() && $include_link ) {
			...
			$image = '<span class="image">' . $image_markup . '</span> ';
		}
		...
		if ( $include_author ) {
			...
			$author = apply_filters( 'display_posts_shortcode_author', ' <span class="author">by ' . sprintf(
		...
		$output = '<' . $inner_wrapper . ' class="' . implode( ' ', $class ) . '">' . $image . $title . '<div class="bh-wrapper-date-cat">' . $author . $date . $category_display_text . '</div>' . $excerpt . $content . '</' . $inner_wrapper . '>';
		 ...
		 */
		$inner .= apply_filters( 'display_posts_shortcode_output', $output, $original_atts, $image, $title, $author, $date, $excerpt, $inner_wrapper, $content, $class, $category_display_text );
	...
	$bh_append_infinite_scroll = '';
	if(isset($bh_infinite_scroll) && $bh_infinite_scroll) {
		$bhDisplayPostCategory = '';
		if ( ! empty( $category_id ) ) {
			$bh_display_post_category = get_category($category_id)->slug;
		} else {
			$bh_display_post_category = $category;
		}

		$bh_append_infinite_scroll = '<script>
				var bhDisplayPostsInfinitePostNotIn = '.json_encode($bh_post_ids).';
				var bhDisplayPostsInfiniteCategorySlug = "'.$bh_display_post_category.'";
			</script><div id="hook-infinite" style="width:100%;height:1pt;"></div>';
	}
	...
	$return .= $open . $inner . $close . $bh_append_infinite_scroll;
