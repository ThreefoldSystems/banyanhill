<?php
/**
 * Plugin Update Checker Library 3.1 - Modified by TFS
 * http://w-shadow.com/
 * 
...
class PluginUpdateChecker_3_1 {
	...
	public function __construct($metadataUrl, $pluginFile, $slug = '', $checkPeriod = 12, $optionName = '', $muPluginFile = ''){
		...
		if ( empty($this->slug) ) {
			// Customized by threefoldsystems
			$this->slug = strtok( $this->pluginFile, '/' );
		}
	...
	protected function installHooks(){
		...
		add_action('all_admin_notices', array($this, 'handleManualCheckNotice'));
	...
	public function handleManualCheck() {
		$get_puc_check_for_updates = ( ! empty( $_GET[ 'puc_check_for_updates' ] ) ? sanitize_text_field( $_GET[ 'puc_check_for_updates' ] ) : '' );
		$get_puc_slug = ( ! empty( $_GET[ 'puc_slug' ] ) ? sanitize_text_field( $_GET[ 'puc_slug' ] ) : '' );

		$shouldCheck = $get_puc_check_for_updates && $get_puc_slug && $get_puc_slug == $this->slug && current_user_can('update_plugins') && check_admin_referer('puc_check_for_updates');
	 ...
	 * @return void
	 */
	public function handleManualCheckNotice() {
		$plugin_header = $this->getPluginHeader();

		// Check for transient plugin update version.
		// If transient exists and it matches version of current plugin (updated since last check) -> reset transients
		if ( false !== ( $plugin_version_transient = get_transient( 'tfs_mw_plugin_update_check_version_' . $this->slug ) ) ) {
			// Check if versions match (has it been updated sicne last check?)
			if ( $plugin_version_transient == $plugin_header[ 'Version' ] ) {
				// Delete this transient so this check doesn't occur again until an actual update is available
				delete_transient( 'tfs_mw_plugin_update_check_version_' . $this->slug );

				// Reset transient so no requests are made for another 24 hours
				set_transient( 'tfs_mw_plugin_update_check_' . $this->slug, null, 24 * HOUR_IN_SECONDS );
			}
		}

		// Check for transient plugin update. Don't make another request to git if it's already done within the last day
		if ( false === ( $update = get_transient( 'tfs_mw_plugin_update_check_' . $this->slug ) ) ) {
			// If no transient, check for new version + check transient
			$update = $this->checkForUpdates();

			// Put the results in a transient. Expire after 24 hours.
			// Can be empty/null, we can check later. For now we just need to have the transient so no more requests are made for 24 hours.
			set_transient( 'tfs_mw_plugin_update_check_' . $this->slug, $update, 24 * HOUR_IN_SECONDS );

			// If an actual update exists, store version of the new plugin in a transient
			if ( $update ) {
				set_transient( 'tfs_mw_plugin_update_check_version_' . $this->slug, $update->version, 24 * HOUR_IN_SECONDS );
			}
		}

		if ( $update ) {
			?>
			<div class="update-nag update-message notice inline notice-warning notice-alt is-dismissable" style="display: block; padding: 1px 12px; margin-left: 2px;-webkit-box-shadow: 0 1px 1px 0 rgba(0,0,0,.1);box-shadow: 0 1px 1px 0 rgba(0,0,0,.1);">
				<p>
					There is a new version of <strong><?php echo $plugin_header[ 'Name' ]; ?></strong> available. You can update this plugin <a href="<?php echo admin_url( 'plugins.php' ); ?>">here</a>.
				</p>
			</div>
			<?php
		}
	...
	public function displayManualCheckResult() {
		$get_puc_update_check_result = ( ! empty( $_GET[ 'puc_update_check_result' ] ) ? sanitize_text_field( $_GET[ 'puc_update_check_result' ] ) : '' );
		$get_puc_slug = ( ! empty( $_GET[ 'puc_slug' ] ) ? sanitize_text_field( $_GET[ 'puc_slug' ] ) : '' );

		if ( $get_puc_update_check_result && $get_puc_slug && ($get_puc_slug == $this->slug) ) {
			$status = strval($get_puc_update_check_result);
			if ( $status == 'no_update' ) {
