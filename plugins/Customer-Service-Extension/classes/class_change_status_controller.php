    ...
    public function csd_ext_change_status () {
        ...
        if ( !empty( $_POST['data'] ) ) {
            ...
            if ( !empty( $data['email_address'] ) ) {
                $content['email_address'] = sanitize_text_field($data['email_address']);
            }
            if ( !empty( $data['reports_link'] ) ) {
                $content['reports_link'] = sanitize_text_field($data['reports_link']);
            }			
            ...
            if ( !empty( $data['status_flow_index'] ) ) {
                ...
                if ( $this->display_order[$index] === 'refund_slider' ) {
                    ...
                    if ( !empty( $content['sub_ref'] ) ) {
						// Make exception for SVC
						if ( $content['pubcode'] === 'SVC' ) {
							$return['html'] = $this->display_class->display_frontend( 'change_status/customer_services',
								$content );
							die( json_encode($return) );							
						} else {
        ...
        if ( is_array($subscriptions) ) {
            foreach ( $subscriptions as $subscription ) {
                if ( $subscription->id->subRef === $subref ) {
					...
					$trm_no = $subscription->termNumber;
                }
            }
        }

        if( !is_wp_error($customer) && !empty($pubcode) && !empty($subref) ) {
            $cancel = $this->middleware->core->mw->cancelSubscription( $pubcode, $customer, $subref, 'API-TRIGGER-CIR-GET', $trm_no );
