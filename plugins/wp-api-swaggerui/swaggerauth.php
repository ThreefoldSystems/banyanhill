...
class SwaggerAuth {
	...
	public function appendSwaggerAuth( $auth ) {
		...
		if (strpos($_SERVER['SERVER_NAME'], 'dev.banyanhill.com') !== false) {
			$auth['basic'] = array(
				'type' => 'basic'
			);

			if (isset($_GET['prefix']) && $_GET['prefix'] == '/bhapp/v1') {
				$auth['Application-Authorization'] = array(
					'type' => 'apiKey',
		...
		} else if (isset($_GET['prefix']) && $_GET['prefix'] == '/bhapp/v1') {
			$auth['basic'] = array(
				'type' => 'basic'
			);
		} else {
