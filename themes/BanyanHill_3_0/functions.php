<?php
	...
	wp_enqueue_script('bh-extras-global', (get_stylesheet_directory_uri() . '/js/bh-extras.js?date=2020-03-09-00'));
	...
	function bh_rest_api_init_block_list() {
		...
		$pathClean = ltrim($pathClean, '/');

		if ($pathClean == 'wp-json') {
	...
	function bh_check_refresh_data() {
      ...
      $origin = '';
		...
		$date_last_login = get_user_meta($user_id, 'bh_last_login_datetime', true);

		$rs = $db->query("SELECT `id` FROM `user_cache_buster` WHERE `origin`='".$origin."' AND `bust` < '".gmdate('Y-m-d H:i:s')."' AND `bust` > '".$date_last_login."' LIMIT 1");
		if($rs->num_rows > 0 || empty($date_last_login)){
			update_usermeta($user_id, 'bh_last_login_datetime', gmdate('Y-m-d H:i:s'));

			if (function_exists('agora')) {
				$agora = agora();

				$agora_data = get_user_meta($user_id, 'agora_middleware_aggregate_data', true);

				if(isset($agora_data->accounts) &&
					isset($agora_data->accounts[0]) &&
					isset($agora_data->accounts[0]->id) &&
					isset($agora_data->accounts[0]->id->userName) &&
					isset($agora_data->accounts[0]->password) &&
					!empty($agora_data->accounts[0]->id->userName) &&
					!empty($agora_data->accounts[0]->password)
				) {
						$agora_aggregate_data_new = $agora->mw->get_aggregate_data_by_login($agora_data->accounts[0]->id->userName, $agora_data->accounts[0]->password);

						if(isset($agora_aggregate_data_new->accounts) &&
							isset($agora_aggregate_data_new->accounts[0]) &&
							isset($agora_aggregate_data_new->accounts[0]->id) &&
							isset($agora_aggregate_data_new->accounts[0]->id->userName) &&
							isset($agora_aggregate_data_new->accounts[0]->password) &&
							!empty($agora_aggregate_data_new->accounts[0]->id->userName) &&
							!empty($agora_aggregate_data_new->accounts[0]->password)
						) {
								update_user_meta($user_id, 'agora_middleware_aggregate_data', $agora_aggregate_data_new);
						}
				}
			}
		}
	...
	function bh_symbol() {
		$symbol_info = do_shortcode('[rwc_horizontal name="'.$_POST['symbol'].'"]');
	...
	add_action( 'wp_ajax_bh_symbol', 'bh_symbol' );
	add_action( 'wp_ajax_nopriv_bh_symbol', 'bh_symbol' );

	function bh_stock_ticker_top_five() {
		$stockdb = bh_extras_db();
		if(!$stockdb) {
			return [];
		}
	...
	function be_ajax_load_more() {
		...
		if( $loop->have_posts() ) {
			while( $loop->have_posts() ) {
				...
				$post_authors[]= get_the_author_meta('display_name', $loop->post->post_author);
				$post_authors_url[]= get_author_posts_url($loop->post->post_author);
		...
		$data = array(
			...
			'post_authors_url' => $post_authors_url,
			'loop'=> $loop
		);
		wp_reset_postdata();
		wp_send_json_success( $data );
	...
	function check_auth_by_post_id( $post_id, $post_permalink ) {
		...
		if ( class_exists( 'agora_auth_container' ) ) {
			...
			if ( ! empty( $query_string ) ) {
				...
				$query_string = '?redirect_to=' . urlencode( $post_permalink ) . '&op=refresh';
			}
	...
	// Ajax callback - load ePortfolio content
	function load_eportfolio() {
		$auth_container = new agora_auth_container( $_POST['pid'] );
		$auth_container = apply_filters('agora_middleware_check_permission', $auth_container);

		if ( $auth_container->is_allowed() ) {
			$data_return = array(
				'success' => true,
				'data' => apply_filters('the_content', get_post_field('post_content', $_POST['pid']))
			);
		} else {
			$data_return = array(
				'success' => false,
				'data' => 'You are not authorized to view this content'
			);
		}
	...
	add_action("wp_ajax_load_eportfolio","load_eportfolio");
	...
	// Ajax callback - load more archives for folded prods
	function load_fold_archives() {
		$terms_id = $_POST['term_id'];
...
//			'posts_per_page' => -1,
	...
	add_action("wp_ajax_nopriv_load_fold_archives","load_fold_archives");
	add_action("wp_ajax_load_fold_archives","load_fold_archives");
						...
						<div class="mob_inner">
							<h2>
								<a class="pdf-block"
									 ...
									 href="<?php echo get_post_meta(get_the_ID(), 'pdf_link', true) ?>"><?php if ( get_post_meta(get_the_ID(), 'page_title', true) ) {
										echo get_post_meta(get_the_ID(), 'page_title', true);
									} else {
										the_title();
									} ?>
...
//	}
	...
	add_shortcode( 'bh-check-login-refresh', 'bh_check_login_refresh' );
	function bh_check_login_refresh () {
		if(is_user_logged_in()){
			if(isset($_GET['op']) && isset($_GET['redirect_to']) && $_GET['op'] == 'refresh' ) {
				print('<p style="color: red;padding: 10px;text-align: center;border: 1px solid red;">Are you looking for a new subscription? Please click <a href="/customer-login/?redirect_to='.urlencode($_GET['redirect_to']).'&op=reloadsession">here</a> to refresh your session.</p>');
			}
		}
	}

	add_action('template_redirect', 'bh_check_login_reloadsession');
	function bh_check_login_reloadsession(){
		if(is_user_logged_in()){
			if(isset($_GET['op']) && isset($_GET['redirect_to']) && $_GET['op'] == 'reloadsession' ) {
				wp_destroy_current_session();
		    wp_clear_auth_cookie();
		    wp_set_current_user( 0 );

				wp_redirect( '/customer-login/?redirect_to='.urlencode($_GET['redirect_to']) );
				exit();
			}
		}
	}
	...
	// Page Metadata for Lytics
	function get_page_meta_data_var() {
	?>
		<script type="text/javascript">
		  !function (l, a) { a.liosetup = a.liosetup || {}, a.liosetup.callback = a.liosetup.callback || [], a.liosetup.addEntityLoadedCallback = function (l, o) { if ("function" == typeof a.liosetup.callback) { var i = []; i.push(a.liosetup.callback), a.liosetup.callback = i } a.lio && a.lio.loaded ? l(a.lio.data) : o ? a.liosetup.callback.unshift(l) : a.liosetup.callback.push(l) } }(document, window);
...
//			}
	...
	add_action('wp_footer', 'get_page_meta_data_var');
	...
	function set_last_login($login) {
		...
		update_usermeta( $user->ID, 'bh_last_login_datetime', gmdate('Y-m-d H:i:s') );
	...
	add_action( 'wp_ajax_bh_toggle_avatar', 'bh_toggle_avatar' );
	add_action( 'wp_ajax_nopriv_bh_toggle_avatar', 'bh_toggle_avatar' );
	add_shortcode( 'bh_display_avatar', 'bh_display_avatar' );

	function bh_display_avatar() {
		$user = wp_get_current_user();
		
		$current_data = get_user_meta( $user->ID , 'display_avatar', true );
		
		return '<label class="switch"><input type="checkbox" value="' . $current_data . '" class="display_avatar" ' . ($current_data === 'true' ? 'checked' : '') . ' /><span class="slider round"></span></label>';
	}

	function bh_toggle_avatar() {
		...
		if ($current_data !== $payload['display_avatar']) {
			update_user_meta( $user->ID, 'display_avatar', $payload['display_avatar'] );
			
			$gravatar = $payload['display_avatar'] === 'true' ? get_avatar_url($user->ID) : get_avatar_url( $user->ID, array(
				'force_default' => true,
				'size' => '150'
				) 
			);
		...
		echo json_encode( array('status' => 'success', 'content' => '<label class="switch"><input type="checkbox" value="' . $payload['display_avatar'] . '" class="display_avatar" ' . ($payload['display_avatar'] === 'true' ? 'checked' : '') . '><span class="slider round"></span></label>', 'image' => $gravatar ) );
	...
	/**
	 * wp_is_mobile() Alternative
	 * AWS Cloudfront sends a default User-Agent string, AWS console must be configured to send these headers:
	 * CloudFront-Is-Desktop-Viewer
	 * CloudFront-Is-Mobile-Viewer
	 * https://support.amimoto-ami.com/en/articles/1355401-detect-and-handle-user-agent-with-wordpress-and-cloudfront
	 */
	function cf_is_mobile() {
		$is_mobile = function_exists('wp_is_mobile') && wp_is_mobile();

		// returns true if the CloudFront assumes the browser is a smartphone
	...
	// A function to store the correct IP when on cloudfront servers
	function eform_update_user_IP( IPT_FSQM_Form_Elements_Data $data ) {
		// update the database
		/** @var WPDB $wpdb  */
	...
	add_filter( 'ipt_fsqm_hook_save_insert', 'eform_update_user_IP', 10, 1 );
