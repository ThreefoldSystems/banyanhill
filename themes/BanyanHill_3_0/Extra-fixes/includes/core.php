...
function extra_init_walker_nav_menu() {
	class Extra_Walker_Nav_Menu extends Walker_Nav_Menu {
		...
		function mega_item_featured_2( $item, $args, $posts ) {
			...
			/*
			 * JW Fix: Loop through the 10 most recent category posts, sort and format for correct section.
			 * 8/21/2108 Change to loop through 5 and display regardless
			 */	
			
			for ( $post_count = 0; $post_count <= 5; $post_count++ ) {
				if ( !isset( $posts->posts[ $post_count ] ) ) {
					break;
				}
				$post = $posts->posts[ $post_count ];

				// Get most recent regardless of featured status
				if (/*get_post_field('_extra_featured_post', $post->ID) === '1' &&*/ $featured_posts_count < $featured_posts_limit) {
					...
					$featured_posts .= sprintf('
						<li>
							<h2 class="title">%1$s</h2>
							<div class="featured-post">
								%2$s
								<h2><a href="%5$s">%3$s</a></h2>
								<div class="post-meta">
									%4$s
								</div>
							</div>
				...
				} else if ($related_posts_count < $recent_posts_limit) {
					$thumb = et_extra_get_post_thumb(array(
						'post_id'   => $post->ID,
						...
						<li class="recent-post">
							<div class="post-content">
								...
								<div class="post-meta">
									...
									<div class="post-meta-byline">%4$s</div>
								</div>
							</div>
						</li>',
						$thumb,
