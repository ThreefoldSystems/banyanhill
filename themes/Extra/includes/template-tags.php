...
function et_thumb_as_style_background() {
	...
	if ( ! empty( $thumb_src ) ) {
		echo 'data-src="' . esc_attr( $thumb_src ) . '"';
		// JW Edit 06/25/2018
...
function extra_blog_feed_get_content() {
	...
	// JW Fix 6/27/2018
	// Fix pagination not taking displayed posts into account 
	$et_displayed_ids = isset( $_POST['et_displayed_ids'] ) ? $_POST['et_displayed_ids'] : array();
	...
	// JW Fix 6/24/2019
	// Revert ignored posts for category page view
	if (!empty( $tax_query )) {
		// JW Fix 6/27/2018
		// Fix pagination not taking displayed posts into account 		
		$args['post__not_in'] = array($et_displayed_ids);
	} else {
		$args['post__not_in'] = $et_displayed_ids;
	}
...
function et_extra_display_post_meta( $args = array() ) {
	...
	$output = implode( '<span class="post-meta-separator"> | </span>', $meta_pieces );
...
function extra_get_column_thumbnail_size() {
	...
	if ( is_singular( 'post' ) || is_singular( EXTRA_PROJECT_POST_TYPE ) || is_page() ) {
		$size = wp_is_mobile() ? 'extra-image-medium' : 'extra-image-huge';
	} else {
